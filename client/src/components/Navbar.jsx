...
export default function Navbar() {
  ...
  return (
    <>
      <nav style={{ backgroundColor: NAV_BG }} className="fixed top-0 left-0 right-0 z-40 text-white shadow-lg">
        {/* Main bar */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="hidden sm:flex items-center gap-3 text-sm">
            <Link to="/cart" className="relative text-gray-300 hover:text-white transition-colors p-1.5">
              ...
              {items.length > 0 && (
                <span style={{ backgroundColor: PRIMARY }} className="absolute -top-0.5 -right-0.5 text-white text-xs font-bold w-4 h-4 rounded-full flex items-center justify-center leading-none">{items.length}</span>
              )}
            </Link>
            ...
            {user ? (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(v => !v)} className="flex items-center gap-2 hover:opacity-90 transition-opacity">
                  <div style={{ backgroundColor: PRIMARY }} className="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold select-none">{initials}</div>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                      <div className="flex items-center gap-3">
                        <div style={{ backgroundColor: PRIMARY }} className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">{initials}</div>
              ...
              <>
                ...
                <button onClick={() => openModal('register')} style={{ backgroundColor: PRIMARY }} className="text-white px-4 py-1.5 rounded font-medium transition-colors hover:opacity-90">Get Started</button>
              </>
            )}
          </div>
        ...
        <div className="hidden sm:block border-t border-white/10">
          <div className="max-w-7xl mx-auto px-6 flex gap-6 text-xs text-gray-400 overflow-x-auto py-2">
            <Link to="/" className="whitespace-nowrap hover:text-white transition-colors pb-0.5 border-b border-transparent">All Items</Link>
            {categories.map(cat => (
              <Link key={cat.id} to={`/?category=${cat.slug}`} className="whitespace-nowrap hover:text-white transition-colors pb-0.5 border-b border-transparent capitalize">{cat.name}</Link>
            ))}
          </div>
        </div>
      </nav>
      ...
      <div style={{ backgroundColor: NAV_BG }} className={`fixed top-0 right-0 h-full w-72 z-50 sm:hidden flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
          {user ? (
            <>
              <div className="flex items-center gap-3 px-3 py-3 mb-2 bg-white/5 rounded-xl">
                <div style={{ backgroundColor: PRIMARY }} className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold shrink-0">{initials}</div>
                <div className="min-w-0">
              ...
              {user.role === 'admin' && (
                <>
                  ...
                  <Link to="/admin" onClick={closeSidebar} className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm text-gray-300 hover:text-white hover:bg-white/10 transition-all">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                    Admin Dashboard
                  </Link>
                </>
            ...
            <>
              <button onClick={() => { openModal('login'); closeSidebar(); }} className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm text-gray-300 hover:text-white hover:bg-white/10 transition-all w-full text-left">
              ...
              <button onClick={() => { openModal('register'); closeSidebar(); }} style={{ backgroundColor: PRIMARY }} className="flex items-center justify-center gap-2 w-full px-3 py-3 rounded-xl text-sm text-white font-semibold transition-all hover:opacity-90">
                Get Started
              </button>
            </>
          )}
          ...
          {categories.length > 0 && (
            <>
              ...
              <Link to="/" onClick={closeSidebar} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-300 hover:text-white hover:bg-white/10 transition-all">All Items</Link>
              {categories.map(cat => (
                <Link key={cat.id} to={`/?category=${cat.slug}`} onClick={closeSidebar} className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-300 hover:text-white hover:bg-white/10 transition-all capitalize">
                  {cat.name}
                </Link>
              ))}
            </>
        ...
        {user && (
          <div className="px-4 py-4 border-t border-white/10">
            <button onClick={handleLogout} className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-all w-full text-left">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
              Sign Out
            </button>
          </div>
