      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-[#1a1a2e] text-white shadow-lg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="hidden sm:flex items-center gap-3 text-sm">
            <Link to="/cart" className="relative text-gray-300 hover:text-white transition-colors p-1.5">
              ...
              {items.length > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-green-500 text-white text-xs font-bold w-4 h-4 rounded-full flex items-center justify-center leading-none">{items.length}</span>
              )}
            </Link>
            ...
            {user ? (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setDropdownOpen(v => !v)} className="flex items-center gap-2 hover:opacity-90 transition-opacity">
                  <div className="w-8 h-8 rounded-full bg-green-500 flex items-center justify-center text-white text-sm font-bold select-none">{initials}</div>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-100 bg-gray-50">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-green-500 flex items-center justify-center text-white text-sm font-bold shrink-0">{initials}</div>
              ...
              <>
                ...
                <button onClick={() => openModal('register')} className="bg-green-500 hover:bg-green-600 text-white px-4 py-1.5 rounded font-medium transition-colors">Get Started</button>
              </>
            )}
          </div>
        ...
        <div className="hidden sm:block border-t border-white/10">
          <div className="max-w-7xl mx-auto px-6 flex gap-6 text-xs text-gray-400 overflow-x-auto py-2">
            <Link to="/" className="whitespace-nowrap hover:text-white transition-colors pb-0.5 border-b border-transparent hover:border-green-400">All Items</Link>
            {categories.map(cat => (
              <Link key={cat.id} to={`/?category=${cat.slug}`} className="whitespace-nowrap hover:text-white transition-colors pb-0.5 border-b border-transparent hover:border-green-400 capitalize">{cat.name}</Link>
            ))}
          </div>
        </div>
      </nav>
          ...
          {user ? (
            <>
              ...
              <div className="flex items-center gap-3 px-3 py-3 mb-2 bg-white/5 rounded-xl">
                <div className="w-10 h-10 rounded-full bg-green-500 flex items-center justify-center text-white font-bold shrink-0">{initials}</div>
                <div className="min-w-0">
              ...
              <Link to="/cart" onClick={closeSidebar}
                ...
                Cart {items.length > 0 && <span className="ml-auto bg-green-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{items.length}</span>}
              </Link>
            ...
            <>
              ...
              <button onClick={() => { openModal('register'); closeSidebar(); }}
                className="flex items-center justify-center gap-2 w-full px-3 py-3 rounded-xl text-sm bg-green-500 hover:bg-green-600 text-white font-semibold transition-all">
                Get Started
              </button>
            </>
          )}
