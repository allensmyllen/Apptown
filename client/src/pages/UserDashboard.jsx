...
const inputCls = 'w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-300';
...
export default function UserDashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div style={{ backgroundColor: '#0D0D1A' }} className="pt-8 pb-0 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-end gap-5 pb-5">
            {/* Avatar */}
            <div
              style={{ backgroundColor: PRIMARY }}
              className="w-16 h-16 rounded-xl flex items-center justify-center text-white text-2xl font-bold shrink-0 shadow-lg"
            >
              {initials}
            </div>
            {/* Name + meta */}
            <div className="pb-1">
              <h1 className="text-white text-xl font-bold leading-tight capitalize">{displayName}</h1>
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                {joinDate && <span className="text-gray-400 text-xs">Member since {joinDate}</span>}
                <span className="text-gray-600 text-xs">·</span>
                <span className="text-gray-400 text-xs">{orders.length} purchase{orders.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
            {/* Right — total purchases stat */}
            <div className="ml-auto pb-1 text-right hidden sm:block">
              <p className="text-gray-400 text-xs uppercase tracking-wider">Purchases</p>
          ...
          <div className="flex gap-0 border-b border-white/10 overflow-x-auto">
            {TABS.map(({ key, label }) => (
              <button
                ...
                style={tab === key ? { borderBottomColor: PRIMARY } : {}}
              >
                {label}
                {key === 'downloads' && orders.length > 0 && (
                  <span className="ml-1.5 text-xs bg-white/10 text-gray-300 px-1.5 py-0.5 rounded-full">{orders.length}</span>
                )}
                {key === 'support' && supportLicenses.length > 0 && (
                  <span className="ml-1.5 text-xs bg-white/10 text-gray-300 px-1.5 py-0.5 rounded-full">{supportLicenses.length}</span>
                )}
              </button>
            ))}
          </div>
        ...
        {tab === 'downloads' && (
          loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
              ...
              <Link to="/" style={{ backgroundColor: PRIMARY }} className="text-white text-sm font-semibold px-5 py-2 rounded-lg inline-block hover:opacity-90 transition-opacity">
                Browse Marketplace
              </Link>
            </div>
          ) : (
        ...
        {tab === 'support' && (
          loading ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-16 text-center">
              ...
              <Link to="/" style={{ backgroundColor: PRIMARY }} className="text-white text-sm font-semibold px-5 py-2 rounded-lg inline-block hover:opacity-90 transition-opacity">Browse Products</Link>
            </div>
          ) : (
            <div className="space-y-3">
              {supportLicenses.map(sl => (
                <div key={sl.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <Link to="/support" className="text-xs font-semibold border rounded-lg px-3 py-1.5 transition-colors hover:opacity-80" style={{ color: PRIMARY, borderColor: PRIMARY, backgroundColor: '#eff6ff' }}>
                      Use License
                    </Link>
                  </div>
          ...
          <div className="max-w-2xl">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <form onSubmit={handleProfileSave} className="space-y-4">
                ...
                <button type="submit" disabled={savingProfile} style={{ backgroundColor: PRIMARY }} className="text-white px-6 py-2.5 rounded-lg text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60">
          ...
          <div className="max-w-md">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              ...
              <form onSubmit={handlePasswordSave} className="space-y-4">
                ...
                <button type="submit" disabled={savingPw} style={{ backgroundColor: PRIMARY }} className="text-white px-6 py-2.5 rounded-lg text-sm font-semibold transition-opacity hover:opacity-90 disabled:opacity-60">
