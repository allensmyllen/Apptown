...
const CATEGORY_GRADIENTS = {
  ...
  script: 'from-amber-500 to-orange-600',
...
const CATEGORY_COLORS = {
  ...
  script: 'bg-amber-50 text-amber-700 border-amber-100',
    ...
    <span className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map(i => (
        <svg key={i}
          ...
          className={`w-4 h-4 transition-colors ${interactive ? 'cursor-pointer' : ''} ${i <= Math.round(rating) ? 'text-amber-400' : 'text-gray-200'}`}
